...
def _setup_id(symbol, sel_tf, poi_tf, direction, decision_ts, entry, sl, tp):
    """ID estable para parear experimentos de research sin afectar la senal."""
...
def _run_pass(symbol, sel_tf, htf_series, htf_ts):
    ...
    for i in range(start, len(sel) - 1):
        ...
        # Enriquece unicamente el artefacto gitignored de research para poder
        # re-simular exits y auditar causalidad; no participa en la senal viva.
        rec = {
