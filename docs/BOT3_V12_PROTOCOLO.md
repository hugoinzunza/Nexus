# Bot3.v12 — Protocolo pre-registrado CANDIDATO (ciclo del candidato, cerrado)
...
NO CONFORME) y v11 candidato
(`bf315275ffd7699c54672e5cfe29e601906a7232711af97ec2ae37b9df9363ef`,
NO CONFORME) con sus informes.

v12 reemplaza íntegramente a v11 (que no se edita). Alcance idéntico:
...
- **Partición estricta de eventos** (ningún evento puede emitirse fuera de
  ...
  - en `orden_viva`: `orden_creada`, `orden_cancelada`,
    ...
    **evento terminal ÚNICO y excluyente** (exactamente uno cierra la orden):
    `fill` (→ posición), `gap_ambiguo` (abstención),
    `confirmada_sin_fill` SOLO por agotamiento del deadline total, y
    `orden_cancelada` SOLO por las demás causas (`direccion`, `hueco_m15`).
    Jamás se emiten dos terminales para la misma orden; el timestamp del
    terminal es el `close_time` de la vela en que se determina y el estado
    resultante es siempre `flat`.
...
1. CANDIDATO: se congela como v12 con la conformidad acotada; su SHA-256
   pasa a ser el nuevo `contrato_hash`. Cambio posterior = v13.
2. Cambiar `contrato_hash` cambia TODAS las identidades del ledger: v12 debe
...
Conformidad acotada → congelamiento v12 → implementar B-2 → implementar
