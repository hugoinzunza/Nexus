# Bot3.v13 — Protocolo pre-registrado CANDIDATO (ciclo del candidato, cerrado)
...
NO CONFORME), v11 candidato
(`bf315275ffd7699c54672e5cfe29e601906a7232711af97ec2ae37b9df9363ef`,
NO CONFORME) y v12 candidato
(`818dc60e592af9a0bde9112adf53ba9a12810c759ed52f90bb6d2be5d9ae43d7`,
NO CONFORME por una contradicción registral) con sus informes.

**Único cambio respecto de v12:** la tabla de estado resultante por evento
terminal (abajo), que en v12 decía "siempre `flat`" y contradecía
`fill → posicion`. Nada más cambia.

v13 reemplaza íntegramente a v12 (que no se edita). Alcance idéntico:
...
- **Partición estricta de eventos** (ningún evento puede emitirse fuera de
  ...
  - en `orden_viva`: `orden_creada`, `orden_cancelada`,
    ...
    **evento terminal ÚNICO y excluyente** (exactamente uno cierra la
    orden), con su estado resultante EXPLÍCITO:

    | Terminal | Causa | Estado resultante |
    |---|---|---|
    | `fill` | cruce del nivel (§4.5) | **`posicion`** (o `salida_detectada` si fill+STOP, CF-20) |
    | `gap_ambiguo` | apertura más allá del SL antes del fill | `flat` |
    | `confirmada_sin_fill` | SOLO agotamiento del deadline total | `flat` |
    | `orden_cancelada` | SOLO `direccion` o `hueco_m15` | `flat` |

    Jamás se emiten dos terminales para la misma orden; el timestamp del
    terminal es el `close_time` de la vela en que se determina.
...
3. **Zona derivada** (regla del diseño rev.3, ahora normativa):
   - **Desplazamiento** = velas `[j_origen, j_ibos]`, donde `j_origen` es el
     `idx` del ÚLTIMO swing INT M15 con `confirm_idx ≤ j_ibos` e
     `idx < j_ibos`, tomado LITERALMENTE de este lado:
     **trade LONG → swing LOW**; **trade SHORT → swing HIGH**
     (el origen de un desplazamiento alcista es el mínimo desde el que
     partió, y viceversa). Si no existe, no hay zona derivada para ese
     iBOS.
   ...
   - Si no hay OB ni FVG en el desplazamiento, ese iBOS no tiene zona
     derivada.
4. **Orden límite** en el borde proximal de la zona derivada. Dos sellos
   temporales DISTINTOS y obligatorios:
   - `zone_formation_at` = `close_time` de la vela que forma la zona
     derivada (la vela del OB, o la 3ª del FVG). Es histórico y puede ser
     ANTERIOR al iBOS; se registra en el evento como dato.
   - `order_available_at` = `close_time` de `j_ibos`. La orden no puede
     existir antes de que el iBOS confirme, así que ESTE es el instante de
     disponibilidad de la orden y **es el valor que entra como
     `derivada_avail` en la preimagen de `order_id` (CF-9)**.
   Queda prohibido usar `zone_formation_at` en la identidad: dos iBOS
   distintos pueden elegir la misma caja y deben producir órdenes
   distintas.
...
- **b) Orden:** si con las velas cerradas en `T` existe par ganador
  ...
  (c) y (d) NO se evalúan en esta vela.
  **Orden creada exactamente en la vela del deadline** (`T = close(j_toque)
  + 64·15m`): la orden NO queda viva. Se emiten, en la MISMA vela y en este
  orden, `orden_creada` y acto seguido `confirmada_sin_fill`; estado
  `flat`. Nunca llega a la vela siguiente, por lo que no existe la
  ambigüedad fill-vs-deadline.
...
4. **Zona derivada:** el OB del desplazamiento gana sobre cualquier FVG; una
   ...
   mínimo. Vector de identidad: dos iBOS distintos que eligen la MISMA caja
   producen `order_id` DISTINTOS (porque entra `order_available_at`), y
   `zone_formation_at` anterior al iBOS queda registrado sin afectar la
   identidad.
5. **Precedencia 7a:** vector donde la zona derivada se completa en la MISMA
   vela del deadline → se emiten `orden_creada` + `confirmada_sin_fill` en
   esa vela, estado `flat`, y NO se emite `candidato_expirado` ni queda
   orden viva para la vela siguiente.
5-bis. **Terminal único:** por cada orden, exactamente un evento terminal
   (`fill` | `gap_ambiguo` | `confirmada_sin_fill` | `orden_cancelada`),
   con `confirmada_sin_fill` reservado al deadline y `orden_cancelada` a
   dirección/hueco.
...
1. CANDIDATO: se congela como v13 con la conformidad acotada; su SHA-256
   pasa a ser el nuevo `contrato_hash`. Cambio posterior = v14.
2. Cambiar `contrato_hash` cambia TODAS las identidades del ledger: v13 debe
...
Conformidad acotada → congelamiento v13 → implementar B-2 → implementar
